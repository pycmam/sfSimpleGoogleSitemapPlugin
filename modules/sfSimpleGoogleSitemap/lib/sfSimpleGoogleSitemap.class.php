...
class sfSimpleGoogleSitemap {
   ...
   * @return Object[]
   */
  public function queryUsingDoctrine($model_name, $query_method, $limit)
  {
    if (! $query_method) {
        $query = Doctrine::getTable($model_name)->createQuery('a');
    } else {
        $query = Doctrine::getTable($model_name)->$query_method();
    }
